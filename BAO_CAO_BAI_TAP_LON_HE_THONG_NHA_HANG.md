...
2. **Ghi nhận order**: Khách/Phục vụ chọn món, hệ thống tạo `Order` ngay ở trạng thái `KITCHEN_CONFIRMED` (Phương án B: Gửi thẳng xuống Bếp).
...
```mermaid
flowchart LR
    A[1. Khách chọn Bàn] --> B[2. Ghi nhận Order]
    B --> C[3. Màn hình Bếp KDS]
    C --> D[4. Phục vụ Mang món]
    D --> E[5. Thu ngân POS Thanh toán]
```
...
| **III. Tích hợp & Kiểm thử** | 18/07 (T7-OT) + 20-21/07 (T2-T3) | Nguyễn Văn Test, Cả nhóm | 150+ Test Cases, Bug Fix Log, Release Candidate. *(19/07 Chủ Nhật - Nghỉ bắt buộc)* |
...
graph TD
    subgraph System Boundary: Restaurant Management System
        UC_Auth[UC_AUTH_01: Đăng nhập / JWT]
        UC_Profile[UC_AUTH_02: Hồ sơ & Mật khẩu]
        
        UC_CustMenu[UC_CUST_01: Xem Thực đơn]
        UC_CustReserve[UC_CUST_02: Đặt bàn Online]
        UC_CustOrder[UC_CUST_03: Đặt món Giỏ hàng]
        UC_CustReview[UC_CUST_04: Viết Đánh giá]
        
        UC_WaitTable[UC_WAIT_01: Quản lý Bàn ăn]
        UC_WaitOrder[UC_WAIT_02: Gọi món tại bàn]
        
        UC_ChefKDS[UC_CHEF_01: Màn hình Bếp KDS]
        UC_ChefStatus[UC_CHEF_02: Cập nhật Chế biến]
        
        UC_CashPOS[UC_CASH_01: Thanh toán POS]
        UC_CashPromo[UC_CASH_02: Mã Giảm giá & In HD]
        
        UC_MgrMenu[UC_MGR_01: Quản lý Thực đơn]
        UC_MgrInv[UC_MGR_02: Quản lý Kho & NCC]
        UC_MgrReport[UC_MGR_03: Báo cáo Doanh thu]
        
        UC_AdminRBAC[UC_ADM_01: Phân quyền Admin RBAC]
    end

    Actor_Guest((Khách Vãng Lai)) --> UC_CustMenu
    
    Actor_Cust((Khách Hàng)) --> UC_Auth
    Actor_Cust --> UC_Profile
    Actor_Cust --> UC_CustReserve
    Actor_Cust --> UC_CustOrder
    Actor_Cust --> UC_CustReview

    Actor_Wait((Phục Vụ)) --> UC_Auth
    Actor_Wait --> UC_WaitTable
    Actor_Wait --> UC_WaitOrder

    Actor_Chef((Đầu Bếp)) --> UC_Auth
    Actor_Chef --> UC_ChefKDS
    Actor_Chef --> UC_ChefStatus

    Actor_Cashier((Thu Ngân)) --> UC_Auth
    Actor_Cashier --> UC_CashPOS
    Actor_Cashier --> UC_CashPromo

    Actor_Mgr((Quản Lý)) --> UC_Auth
    Actor_Mgr --> UC_MgrMenu
    Actor_Mgr --> UC_MgrInv
    Actor_Mgr --> UC_MgrReport

    Actor_Admin((Admin)) --> UC_Auth
...
| **Luồng sự kiện chính** | 1. Phục vụ chọn Bàn ăn trên sơ đồ sơ đồ bàn (`OCCUPIED`).<br>2. Chọn danh sách món ăn từ thực đơn kèm ghi chú (ví dụ: Không hành, cay vừa).<br>3. Bấm "Gửi Bếp".<br>4. Backend tạo `ORDER` (status `KITCHEN_CONFIRMED`) và các `ORDER_ITEMS` (status `PENDING`) – Đơn bay thẳng xuống Bếp, không cần bước xác nhận trung gian.<br>5. Hệ thống gửi thông báo WebSocket/HTTP Push xuống Màn hình Bếp KDS. |
...
sequenceDiagram
    ...
    participant API as Auth Controller API
    ...
    SVC->>DB: Query User & Verify BCrypt Password
    DB-->>SVC: User Record OK
    SVC->>SVC: Generate JWT Access & Refresh Token
    SVC-->>API: JwtResponseDTO
    API-->>FE: HTTP 200 OK + JWT Tokens
    FE-->>User: Đăng nhập thành công & Direct Dashboard
```
...
sequenceDiagram
    ...
    participant API as Order Controller API
    participant SVC as Order Service
    participant DB as MySQL Database
    participant KDS as Kitchen KDS UI

    Waiter->>FE: Chọn Bàn & Danh sách Món ăn
    Waiter->>FE: Nhấn "Gửi Bếp"
    FE->>API: POST /api/waiter/orders (CreateOrderDTO)
    API->>SVC: Create Order & OrderItems
    SVC->>DB: Save Order (status = KITCHEN_CONFIRMED)
    DB-->>SVC: Success
    SVC-->>API: OrderResponseDTO
    API-->>FE: HTTP 201 Created
    SVC-->>KDS: Push Notification "Có món mới Bàn [X]"
```
...
sequenceDiagram
    ...
    Cashier->>FE: Chọn Đơn hàng Bàn [X]
    FE->>API: GET /api/cashier/orders/{id}/checkout-info
    API->>SVC: Calculate Total, VAT & Points
    SVC-->>FE: Return CheckoutDTO
    Cashier->>FE: Chọn PTTT (VNPay / Cash) & Bấm "Thanh toán"
    FE->>API: POST /api/cashier/invoices/process-payment
    API->>SVC: Process Payment Transaction
    SVC->>DB: Save Invoice (PAID), Update Table (DIRTY)
    DB-->>SVC: Success
    SVC-->>API: InvoiceCompleteDTO
    API-->>FE: In Hóa đơn POS & Cập nhật UI
```
...
erDiagram
    ...
    DINING_TABLES ||--o{ RESERVATIONS : "assigned"
    DINING_TABLES ||--o{ ORDERS : "located"
    
    DISHES }|--|| CATEGORIES : "belongs"
    DISHES ||--o{ ORDER_ITEMS : "included"
    
    ORDERS ||--|{ ORDER_ITEMS : "contains"
    ORDERS ||--o| INVOICES : "generates"
    INVOICES ||--o{ PAYMENTS : "paid via"
    PROMOTIONS ||--o{ INVOICES : "applied"
```
...
12. **Bảng `orders`**:
    ...
    - `status`: VARCHAR(30) DEFAULT 'KITCHEN_CONFIRMED' (`KITCHEN_CONFIRMED`, `COOKING`, `READY`, `SERVED`, `COMPLETED`, `CANCELLED`)
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderResponseDTO createOrder(CreateOrderDTO dto, User waiter) {
        ...
        order.setStatus(OrderStatus.KITCHEN_CONFIRMED); // Phương án B: Gửi thẳng xuống Bếp
