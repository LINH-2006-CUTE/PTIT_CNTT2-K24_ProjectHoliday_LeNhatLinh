...
export default function TableManagement() {
  ...
  return (
    <div className="relative">
      ...
      <div className="bg-gradient-to-r from-[#2C0C11] via-[#4A121A] to-[#2C0C11] text-white rounded-3xl p-6 sm:p-8 mb-8 shadow-2xl relative overflow-hidden border border-[#C5A059]/30">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 relative z-10">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-gray-300 font-light tracking-wide max-w-xl leading-relaxed">
              Theo dõi trực quan 7 trạng thái bàn ăn, hỗ trợ đặt bàn trước, điều phối phục vụ, chuyển bàn và ghép bàn thông minh.
            </p>
          </div>

          <div className="flex flex-wrap sm:flex-nowrap gap-3 items-center shrink-0">
            {/* View Mode Toggle */}
            <div className="flex bg-black/40 p-1 rounded-2xl border border-white/15 backdrop-blur-md">
              <button
                type="button"
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all border flex items-center gap-2 cursor-pointer shadow-md whitespace-nowrap ${
                isMergeMode
            ...
            >
              <svg className="h-4 w-4 text-[#E6C687]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
              <span>{isMergeMode ? 'Hủy Ghép Bàn' : 'Ghép Bàn Ăn'}</span>
            </button>
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-2xl text-xs font-bold uppercase tracking-wider bg-gradient-to-r from-[#C5A059] to-[#E6C687] text-[#2C0C11] hover:from-[#d4b06a] hover:to-[#f0d497] shadow-xl flex items-center gap-2 cursor-pointer transition-all transform hover:-translate-y-0.5 active:translate-y-0 whitespace-nowrap"
            >
      ...
      {/* STATUS LEGEND BAR WITH REALTIME COUNTS & QUICK FILTER */}
      <div className="bg-white rounded-3xl p-4 mb-6 border border-[#C5A059]/20 shadow-xl space-y-3">
        <div className="flex items-center justify-between border-b border-gray-100 pb-2.5">
          <span className="text-xs font-bold uppercase tracking-wider text-[#4A121A] flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#C5A059] inline-block animate-pulse" />
            Sơ Đồ 7 Trạng Thái Trực Quan:
          </span>
          <span className="text-[11px] text-gray-400 font-medium">Bấm vào từng ô trạng thái để lọc nhanh</span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 text-xs">
          {[
            { key: 'AVAILABLE', text: 'Bàn Trống', count: tables.filter(t => t.status === 'AVAILABLE').length, bg: 'bg-emerald-50 text-emerald-800 border-emerald-200', dot: 'bg-emerald-500' },
            { key: 'RESERVED', text: 'Đã Đặt Trước', count: tables.filter(t => t.status === 'RESERVED').length, bg: 'bg-purple-50 text-purple-800 border-purple-200', dot: 'bg-purple-500' },
            { key: 'OCCUPIED', text: 'Đang Phục Vụ', count: tables.filter(t => t.status === 'OCCUPIED').length, bg: 'bg-amber-50 text-amber-900 border-amber-200', dot: 'bg-amber-500' },
            { key: 'DIRTY', text: 'Chờ Dọn', count: tables.filter(t => t.status === 'DIRTY').length, bg: 'bg-orange-50 text-orange-800 border-orange-200', dot: 'bg-orange-500' },
            { key: 'CLEANING', text: 'Đang Vệ Sinh', count: tables.filter(t => t.status === 'CLEANING').length, bg: 'bg-blue-50 text-blue-800 border-blue-200', dot: 'bg-blue-500' },
            { key: 'MAINTENANCE', text: 'Bảo Trì', count: tables.filter(t => t.status === 'MAINTENANCE').length, bg: 'bg-rose-50 text-rose-800 border-rose-200', dot: 'bg-rose-500' },
            { key: 'OUT_OF_SERVICE', text: 'Ngừng Dùng', count: tables.filter(t => t.status === 'OUT_OF_SERVICE').length, bg: 'bg-gray-100 text-gray-700 border-gray-200', dot: 'bg-gray-400' }
          ].map((st) => {
            ...
            return (
              ...
              >
                ...
                <span className="ml-1 font-mono font-bold px-2 py-0.5 rounded-full bg-white/80 text-[11px] border border-current/20 shadow-2xs">
                  {st.count}
                </span>
              </button>
            );
      ...
      <div className="bg-white rounded-3xl p-5 mb-8 border border-[#C5A059]/20 shadow-xl flex flex-col lg:flex-row gap-4 justify-between items-center">
        {/* Search Bar */}
        <div className="w-full lg:w-96 relative">
          ...
          <input
            ...
            className="w-full bg-[#FAF7F2] border border-[#E8E2D9] rounded-2xl pl-10 pr-9 py-2.5 text-xs font-semibold text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#4A121A] focus:ring-2 focus:ring-[#C5A059]/20 transition-all"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
        ...
        <div className="w-full lg:w-auto flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2 bg-[#FAF7F2] px-3.5 py-2 rounded-2xl border border-[#E8E2D9]">
            <span className="text-[10px] font-bold text-[#4A121A] uppercase tracking-wider">Khu vực:</span>
            <select
              value={areaFilter}
          ...
          <div className="flex items-center gap-2 bg-[#FAF7F2] px-3.5 py-2 rounded-2xl border border-[#E8E2D9]">
            <span className="text-[10px] font-bold text-[#4A121A] uppercase tracking-wider">Trạng thái:</span>
            <select
              value={statusFilter}
          ...
          {(search || areaFilter !== 'All' || statusFilter !== 'All') && (
            <button
              ...
              className="px-3.5 py-2 rounded-2xl bg-red-50 text-red-700 text-xs font-bold hover:bg-red-100 transition-all cursor-pointer border border-red-200"
            >
              Xóa Bộ Lọc
            </button>
          )}
