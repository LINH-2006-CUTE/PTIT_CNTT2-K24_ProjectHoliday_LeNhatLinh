...
import { User, LogOut, X, Mail, Phone, Lock, CheckCircle2, Shield } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#FAF7F2] text-[#2B2625] font-sans pb-16">
      ...
      <header className="sticky top-0 z-50 bg-[#4A121A] text-white shadow-md border-b-2 border-[#C5A059]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-20 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {/* Admin User Info Badge Button */}
              <button
                onClick={() => setShowProfileModal(true)}
                className="px-3.5 py-1.5 rounded-2xl bg-white/10 border border-[#C5A059]/40 hover:border-[#C5A059] hover:bg-white/20 transition-all cursor-pointer flex items-center gap-3 shadow-md active:scale-95 group"
                title="Xem Chi Tiết Hồ Sơ & Đăng Xuất Tài Khoản Admin"
              >
                <div className="h-9 w-9 rounded-xl bg-[#C5A059] group-hover:scale-105 transition-transform flex items-center justify-center font-black text-[#4A121A] text-sm shrink-0 shadow-sm border border-amber-300/40">
                  {user?.fullName ? user.fullName.charAt(0).toUpperCase() : <User className="w-5 h-5 text-[#4A121A]" />}
                </div>
                <div className="text-left leading-tight hidden sm:block">
                  <div className="text-xs font-bold text-[#FAF7F2] flex items-center gap-1.5">
                    <span>{user?.fullName || 'System Administrator'}</span>
                    <span className="text-[9px] bg-[#C5A059] text-[#4A121A] px-1.5 py-0.5 rounded-md font-mono font-black uppercase">
                      {user?.roles?.[0] || 'ROLE_ADMIN'}
                    </span>
                  </div>
                  <div className="text-[10px] text-[#C5A059] font-mono mt-0.5">{user?.email || 'admin@restaurant.com'}</div>
                </div>
              </button>
            </div>
          </div>
      ...
      {showProfileModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fadeIn">
          <div className="bg-[#FAF7F2] rounded-[2rem] max-w-md w-full overflow-hidden shadow-[0_25px_60px_-15px_rgba(74,18,26,0.6)] border-2 border-[#C5A059]/60 relative animate-scaleUp">
            
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-[#2E0B11] via-[#4A121A] to-[#360D14] text-white px-6 py-5 border-b border-[#C5A059]/50 flex items-center justify-between shadow-md">
              <div className="flex items-center gap-3.5">
                <div className="h-11 w-11 rounded-2xl bg-gradient-to-br from-[#D9A036] via-[#C5A059] to-[#8C6D37] flex items-center justify-center font-bold text-[#3D0C13] shadow-[0_4px_12px_rgba(197,160,89,0.4)] border border-[#FFE7B3]/60 shrink-0">
                  <User className="w-5.5 h-5.5 text-[#3D0C13]" />
                </div>
                <div>
                  <h3 className="font-bold font-serif text-lg text-[#FAF7F2] tracking-wide">Hồ Sơ Quản Trị Viên</h3>
                  <p className="text-[11px] text-[#D9A036] font-mono tracking-wider font-semibold">TÀI KHOẢN HỆ THỐNG ADMIN</p>
                </div>
              </div>
              <button
                onClick={() => setShowProfileModal(false)}
                className="h-9 w-9 rounded-full bg-white/10 hover:bg-[#C5A059] text-amber-200 hover:text-[#3D0C13] transition-all flex items-center justify-center cursor-pointer border border-white/20 active:scale-90 shadow-sm"
              >
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="p-5 rounded-2xl bg-gradient-to-br from-[#FFFDF9] to-[#F5ECE0] border border-[#C5A059]/40 shadow-sm flex items-center gap-4 relative overflow-hidden group">
                <div className="absolute -right-6 -bottom-6 w-24 h-24 rounded-full bg-[#C5A059]/10 blur-xl pointer-events-none" />
                <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-[#D9A036] via-[#C5A059] to-[#785317] flex items-center justify-center font-black text-white text-2xl shadow-[0_6px_16px_rgba(197,160,89,0.35)] border-2 border-[#FFE7B3] shrink-0 transform group-hover:scale-105 transition-transform">
                  {user?.fullName ? user.fullName.charAt(0).toUpperCase() : 'S'}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold font-serif text-lg text-[#3D0C13] tracking-wide truncate">{user?.fullName || 'System Administrator'}</h4>
                  <p className="text-xs text-[#8C6D37] font-mono truncate mb-2">{user?.email || 'admin@restaurant.com'}</p>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono font-black uppercase tracking-wider bg-gradient-to-r from-[#4A121A] to-[#2E0B11] text-[#D9A036] px-2.5 py-0.5 rounded-lg border border-[#C5A059]/40 shadow-xs">
                      {user?.roles?.[0] || 'ROLE_ADMIN'}
                    </span>
                    <span className="text-xs text-[#614922] font-semibold flex items-center gap-1">
                      <Shield className="w-3.5 h-3.5 text-[#C5A059]" /> Toàn quyền
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                <h4 className="text-[11px] font-bold text-[#8C7A5C] uppercase tracking-widest font-mono flex items-center gap-1.5">
                  <Mail className="w-3.5 h-3.5 text-[#C5A059]" /> Chi tiết cá nhân
                </h4>
                <div className="grid grid-cols-2 gap-2.5 text-xs">
                  <div className="bg-white p-3.5 rounded-2xl border border-[#E8E2D9] hover:border-[#C5A059]/50 transition-all shadow-xs">
                    <span className="text-[10px] font-bold text-[#9E8B6D] uppercase tracking-wider block mb-0.5 font-mono">Họ và tên</span>
                    <span className="font-bold text-[#2B2625] truncate block">{user?.fullName || 'System Administrator'}</span>
                  </div>
                  <div className="bg-white p-3.5 rounded-2xl border border-[#E8E2D9] hover:border-[#C5A059]/50 transition-all shadow-xs">
                    <span className="text-[10px] font-bold text-[#9E8B6D] uppercase tracking-wider block mb-0.5 font-mono">Vai trò chính</span>
                    <span className="font-bold text-amber-800 font-mono">Quản Trị Viên</span>
                  </div>
                  <div className="bg-white p-3.5 rounded-2xl border border-[#E8E2D9] hover:border-[#C5A059]/50 transition-all shadow-xs">
                    <span className="text-[10px] font-bold text-[#9E8B6D] uppercase tracking-wider block mb-0.5 font-mono">Email hệ thống</span>
                    <span className="font-bold text-[#2B2625] font-mono truncate block">{user?.email || 'admin@restaurant.com'}</span>
                  </div>
                  <div className="bg-white p-3.5 rounded-2xl border border-[#E8E2D9] hover:border-[#C5A059]/50 transition-all shadow-xs">
                    <span className="text-[10px] font-bold text-[#9E8B6D] uppercase tracking-wider block mb-0.5 font-mono">Trạng thái</span>
              ...
              <div className="pt-3 border-t border-[#E8E2D9]">
                <button
                  ...
                  className="w-full py-3.5 px-5 rounded-2xl bg-gradient-to-r from-[#8C271E] via-[#A83226] to-[#781F17] hover:from-[#9E2B21] hover:to-[#8C271E] text-white font-bold text-xs shadow-lg hover:shadow-red-900/30 transition-all duration-200 active:scale-95 border border-red-400/30 flex items-center justify-center gap-2.5 uppercase tracking-widest cursor-pointer"
                >
                  <LogOut className="w-4.5 h-4.5 text-white" /> Đăng Xuất Tài Khoản
                </button>
              </div>
