...
export default function CashierPaymentsPage() {
  ...
  const [bankId, setBankId] = useState(localStorage.getItem('letoile_qr_bank_id') || 'MB');
  const [accountNo, setAccountNo] = useState(localStorage.getItem('letoile_qr_account_no') || '0865004828');
  const [accountName, setAccountName] = useState(localStorage.getItem('letoile_qr_account_name') || 'LE NHAT LINH');
  ...
  // Auto-refresh mỗi 5 giây để nhận đơn mới tự động
  useEffect(() => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (res.data && res.data.success && res.data.data && res.data.data.length > 0) {
        ...
        setLastOrderIds(new Set(freshOrders.map(o => o.id)));
        // Chỉ set selectedOrder nếu chưa có
        setSelectedOrder(prev => prev ? prev : freshOrders[0]);
      } else {
        setOrders([]);
  ...
  // Silent refresh (không bật loading spinner)
  const fetchOrdersSilent = async () => {
    try {
      ...
      if (res.data && res.data.success) {
        const freshOrders = res.data.data || [];
        // Đếm đơn mới
        const newIds = freshOrders.map(o => o.id);
        const newOnes = newIds.filter(id => !lastOrderIds.has(id));
        if (newOnes.length > 0) {
          setNewOrderCount(prev => prev + newOnes.length);
        }
        setOrders(freshOrders);
        setLastOrderIds(new Set(newIds));
        // Cập nhật selectedOrder nếu nó bị thay đổi trạng thái
        setSelectedOrder(prev => {
          if (!prev) return freshOrders[0] || null;
    ...
    <div className="min-h-screen bg-[#FAF7F2] text-[#182B2B] font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-[#2A4747]/10 shadow-md flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-[#4E878C] uppercase tracking-widest block mb-1">
              POS Payment Terminal • Tự động cập nhận mỗi 5 giây
            </span>
          ...
          <div className="flex items-center gap-3">
            {newOrderCount > 0 && (
              <div className="flex items-center gap-1.5 bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-2xl text-xs font-bold animate-pulse">
                <BellRing className="w-4 h-4" />
                <span>+{newOrderCount} đơn mới</span>
              </div>
            )}
            <button
              onClick={() => { setNewOrderCount(0); fetchOrders(); }}
              className="flex items-center gap-2 bg-[#2A4747] text-white px-4 py-2.5 rounded-2xl text-xs font-bold hover:bg-[#1a3636] transition-all cursor-pointer"
            >
        ...
        {loading ? (
          ...
          <div className="py-16 text-center bg-white rounded-3xl border border-gray-200 space-y-4">
            <div className="text-6xl">🍽️</div>
            <p className="text-sm font-bold text-gray-500">Chưa có đơn nào cần xử lý</p>
            <p className="text-xs text-gray-400">Hệ thống tự động cập nhận mỗi 5 giây khi khách đặt món</p>
            <button
              onClick={fetchOrders}
              className="mx-auto flex items-center gap-2 bg-[#2A4747] text-white px-5 py-2.5 rounded-2xl text-xs font-bold hover:bg-[#1a3636] transition-all cursor-pointer"
            >
              <RefreshCw className="w-4 h-4" />
