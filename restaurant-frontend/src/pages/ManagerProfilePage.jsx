...
export default function ManagerProfilePage() {
  ...
  const [email, setEmail] = useState(user?.email || 'manager@restaurant.com');
  const [fullName, setFullName] = useState(user?.fullName || 'Trần Hoàng Nam (Quản Lý)');
  const [phone, setPhone] = useState(user?.phone || '+84 944556677');
  const [gender, setGender] = useState(user?.gender || 'MALE');
  const [avatarUrl, setAvatarUrl] = useState(
    user?.avatarUrl || 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=400&auto=format&fit=crop&q=80'
  );
  ...
  const handleEmailChangeAttempt = (e) => {
    ...
    setEmail(val);
    if (val !== (user?.email || 'manager@restaurant.com')) {
  ...
  const handleUpdateProfile = (e) => {
    ...
    setInfoMessage('Cập nhật thông tin Hồ Sơ Quản Lý thành công!');
  };
    ...
    <div className="min-h-screen bg-[#FAF7F2] text-[#0F172A] font-sans">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              <img
                src={avatarUrl}
                alt="Manager Avatar"
                className="h-20 w-20 rounded-2xl object-cover border-2 border-[#C5A059] shadow-lg"
              />
              <label
            ...
            <div>
              <span className="text-xs font-bold text-[#C5A059] uppercase tracking-widest block mb-0.5">
                Executive Manager Profile
              </span>
              <h1 className="text-2xl font-bold font-serif text-[#0F172A]">{fullName}</h1>
              <span className="text-xs text-gray-500 font-mono">
                ROLE_MANAGER (Quản Lý Vận Hành) • {gender === 'MALE' ? 'Nam' : gender === 'FEMALE' ? 'Nữ' : 'Khác'}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-3xl border border-[#1E2A38]/10 shadow-xl space-y-5">
            <h3 className="font-bold font-serif text-lg text-[#0F172A] flex items-center gap-2 border-b border-gray-100 pb-3">
              <User className="w-5 h-5 text-[#C5A059]" /> Cập Nhật Thông Tin Quản Lý
            </h3>
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 text-xs rounded-xl bg-[#FAF7F2] border border-gray-200 focus:outline-none focus:border-[#1E2A38]"
                  />
                </div>
                {emailWarning && (
              ...
              <div>
                <label className="text-xs font-bold text-gray-600 block mb-1">Họ & Tên Quản Lý *</label>
                <div className="relative">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-9 pr-4 py-2.5 text-xs rounded-xl bg-[#FAF7F2] border border-gray-200 focus:outline-none focus:border-[#1E2A38] cursor-pointer"
                  >
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-2xl bg-[#1E2A38] text-white text-xs font-bold hover:bg-[#0F172A] transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer border border-[#C5A059]/40"
              >
                <Save className="w-4 h-4 text-[#C5A059]" /> Lưu Thông Tin Profile
              </button>
            </form>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-[#1E2A38]/10 shadow-xl space-y-5">
            <h3 className="font-bold font-serif text-lg text-[#0F172A] flex items-center gap-2 border-b border-gray-100 pb-3">
              <Lock className="w-5 h-5 text-[#C5A059]" /> Đổi Mật Khẩu Quản Lý
            </h3>
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-2xl bg-[#0F172A] text-white text-xs font-bold hover:bg-[#1E2A38] transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer"
              >
                <ShieldCheck className="w-4 h-4 text-[#C5A059]" /> Cập Nhật Mật Khẩu
              </button>
            </form>
          </div>
