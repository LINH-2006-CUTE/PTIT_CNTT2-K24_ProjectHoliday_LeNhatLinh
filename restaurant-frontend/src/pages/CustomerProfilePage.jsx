...
import { User, Crown, Lock, ShieldCheck, MapPin, Phone, LogOut, Receipt, Sparkles, ChefHat, AlertCircle, Image, Upload, Star, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function CustomerProfilePage() {
  ...
  const validateProfileForm = () => {
    ...
    if (!fullName.trim()) {
      ...
      newErrors.fullName = 'Họ và tên phải gồm ít nhất 2 ký tự.';
    }
    ...
    if (!phone.trim()) {
      ...
      newErrors.phone = 'Số điện thoại không hợp lệ (Phải gồm 10 chữ số đầu 03, 05, 07, 08, 09 hoặc +84).';
    }
  ...
  const validatePasswordForm = () => {
    ...
    if (!newPassword) {
      ...
      newErrors.newPassword = 'Mật khẩu mới phải gồm ít nhất 6 ký tự.';
    }
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (!validateProfileForm()) {
      return setProfileMsg({ text: 'Vui lòng kiểm tra các trường bị lỗi vi phạm bên dưới.', type: 'error' });
    }
  ...
  const handleChangePassword = async (e) => {
    ...
    if (!validatePasswordForm()) {
      return setPasswordMsg({ text: 'Vui lòng kiểm tra các thông tin mật khẩu bên dưới.', type: 'error' });
    }
          ...
          <div className="max-w-3xl mx-auto bg-white rounded-3xl p-8 border border-[#4A2810]/10 shadow-xl animate-fade-in">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-[#FAF7F2] p-4 rounded-2xl border border-[#4A2810]/10">
                <div>
                  <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1 flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5 text-amber-700" />
                    <span>Email (Không thể sửa)</span>
                ...
                <div>
                  <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1 flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5 text-amber-700" />
                    <span>Vai trò Role (Không thể sửa)</span>
                    <Lock className="w-3 h-3 text-amber-700" />
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`form-input text-xs py-2.5 transition-all ${
                      errors.fullName ? 'border-red-500 bg-red-50 focus:border-red-600 ring-2 ring-red-500/20' : ''
                    }`}
                  />
                  {errors.fullName && (
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`form-input text-xs py-2.5 font-mono transition-all ${
                      errors.phone ? 'border-red-500 bg-red-50 focus:border-red-600 ring-2 ring-red-500/20' : ''
                    }`}
                  />
                  {errors.phone && (
                    <span className="text-[11px] text-red-600 font-semibold mt-1 flex items-center gap-1">
              ...
              <div className="p-5 bg-[#FAF7F2] rounded-3xl border border-[#4A2810]/10 space-y-4">
                <label className="text-xs font-bold text-[#4A2810] uppercase tracking-wider flex items-center gap-2">
                  <Image className="w-4 h-4 text-[#8C3A27]" />
                  <span>Thay Đổi Ảnh Đại Diện (Avatar)</span>
                </label>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-5">
                  ...
                  <div className="flex-1 space-y-3 w-full">
                    <div>
                      <span className="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                        <User className="w-3.5 h-3.5 text-[#8C3A27]" />
                        <span>Cách 1: Chọn từ bộ sưu tập Avatar có sẵn</span>
                      </span>
                    ...
                    <div>
                      <span className="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                        <Upload className="w-3.5 h-3.5 text-[#8C3A27]" />
                        <span>Cách 2: Chọn tập tin ảnh từ máy tính</span>
                      </span>
                      <input
                        type="file"
                        accept="image/*"
          ...
          <div className="max-w-3xl mx-auto space-y-8 animate-fade-in">
            ...
            <div className="bg-white rounded-3xl p-7 border border-[#4A2810]/10 shadow-xl space-y-4">
              <h4 className="text-base font-bold font-serif text-[#4A2810] flex items-center gap-2">
                ...
                <span>5 Hạng Thẻ Thành Viên VIP L'ÉCLAT</span>
              </h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-3xl p-6 border border-[#4A2810]/10 shadow-lg space-y-4">
                <h4 className="font-bold font-serif text-sm text-emerald-800 flex items-center gap-2 border-b border-gray-100 pb-2">
                  ...
                  <span>Lịch Sử Tích Điểm (+PTS)</span>
                </h4>
              ...
              <div className="bg-white rounded-3xl p-6 border border-[#4A2810]/10 shadow-lg space-y-4">
                <h4 className="font-bold font-serif text-sm text-[#8C3A27] flex items-center gap-2 border-b border-gray-100 pb-2">
                  ...
                  <span>Lịch Sử Đổi Điểm (-PTS)</span>
                </h4>
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`form-input text-xs py-2.5 font-mono transition-all ${
                    errors.oldPassword ? 'border-red-500 bg-red-50 focus:border-red-600 ring-2 ring-red-500/20' : ''
                  }`}
                />
                {errors.oldPassword && (
                  <span className="text-[11px] text-red-600 font-semibold mt-1 flex items-center gap-1">
              ...
              <div>
                ...
                <input
                  ...
                  className={`form-input text-xs py-2.5 font-mono transition-all ${
                    errors.newPassword ? 'border-red-500 bg-red-50 focus:border-red-600 ring-2 ring-red-500/20' : ''
                  }`}
                />
                {errors.newPassword && (
                  <span className="text-[11px] text-red-600 font-semibold mt-1 flex items-center gap-1">
              ...
              <div>
                ...
                <input
                  ...
                  className={`form-input text-xs py-2.5 font-mono transition-all ${
                    errors.confirmPassword ? 'border-red-500 bg-red-50 focus:border-red-600 ring-2 ring-red-500/20' : ''
                  }`}
                />
