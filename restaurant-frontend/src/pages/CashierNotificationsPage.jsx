  ...
  const handleConfirmNotification = async (id) => {
    try {
      ...
      if (res.data && res.data.success) {
        showToast('✅ Thu Ngân đã xác nhận thanh toán & chuyển đơn món xuống Bếp thành công!');
        fetchNotifications();
      }
    } catch (err) {
      console.error(err);
              ...
              >
                ...
                <div className="pt-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  ...
                  {!n.isConfirmed && (
                    ...
                    >
                      <UserCheck className="w-4 h-4 text-[#4E878C]" />
                      {(n.title || '').includes('#ORD-') || (n.title || '').includes('THANH TOÁN')
                        ? '⚡ Xác Nhận Thanh Toán & Chuyển Bếp'
                        : 'Xác Nhận Đã Đọc & Đã Hiểu'}
                    </button>
                  )}
                </div>
              </div>
