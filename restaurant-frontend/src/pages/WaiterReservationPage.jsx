...
export default function WaiterReservationPage() {
  ...
  const executeCancelReservation = async (resId) => {
    try {
      const res = await api.put(`/api/public/reservations/${resId}/status`, null, {
      ...
      console.error(err);
      showToast('Không thể hủy đặt bàn.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] text-[#222E21] font-sans">
      ...
      {/* CONFIRMATION MODAL FOR WAITER CANCEL RESERVATION */}
      {cancelConfirmId && (
        <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full border border-gray-200 shadow-2xl space-y-6 text-center relative">
            <button
            ...
            <div className="w-16 h-16 rounded-full bg-red-50 text-red-600 flex items-center justify-center mx-auto shadow-inner border border-red-200">
              <XCircle className="w-8 h-8" />
            </div>

            <div className="space-y-2">
              <h3 className="text-xl font-bold font-serif text-[#182B2B]">
                Xác Nhận Hủy Đặt Bàn?
              </h3>
              <p className="text-xs text-gray-600 leading-relaxed">
                Bạn có chắc chắn muốn hủy đặt bàn này? Thao tác sẽ cập nhật trạng thái đơn thành CANCELLED.
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={() => executeCancelReservation(cancelConfirmId)}
                className="flex-1 py-3 rounded-xl bg-red-600 text-white text-xs font-bold hover:bg-red-700 transition-all cursor-pointer shadow-md flex items-center justify-center gap-1.5"
              >
                Xác Nhận Hủy
              </button>
            </div>
