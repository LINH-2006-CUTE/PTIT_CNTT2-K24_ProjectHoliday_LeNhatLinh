...
export default function ChefNotificationPage() {
  ...
  const handleConfirmNotification = async (id) => {
    try {
      const res = await api.put(`/api/staff-notifications/${id}/confirm`, {
        ...
        fullName: user?.fullName || 'Bếp Trưởng (Chef)'
      });

      if (res.data && res.data.success) {
        showToast('Đầu bếp đã xác nhận đã đọc và hiểu chỉ đạo từ Quản lý!');
        fetchNotifications();
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] text-[#3A1C14] font-sans">
      ...
      {/* Toast */}
      {toastMessage && (
        <div className="fixed top-20 right-6 z-[9999] bg-[#7A2E1E] text-white px-5 py-3.5 rounded-2xl shadow-2xl border border-[#D97706]/40 text-xs font-bold uppercase tracking-wider animate-fade-in flex items-center gap-2">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-[#7A2E1E]/10 shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <span className="text-xs font-bold text-[#D97706] uppercase tracking-widest block mb-1">
              Kitchen Alert & Management Broadcast
            </span>
            <h1 className="text-2xl sm:text-3xl font-bold font-serif text-[#3A1C14]">
              Thông Báo Chỉ Đạo Dành Cho Bộ Phận Bếp
            </h1>
          </div>

          <div className="flex items-center gap-2 bg-[#FAF7F2] px-4 py-2 rounded-2xl border border-gray-200 text-xs font-bold text-[#7A2E1E]">
        ...
        {loading ? (
          <div className="text-center py-12 text-gray-500 font-bold text-sm">Đang tải thông báo bếp...</div>
            ...
            {notifications.map((n) => (
              <div
                ...
                className={`bg-white p-6 rounded-3xl border shadow-xl space-y-4 transition-all ${
                  !n.isConfirmed ? 'border-[#D97706] ring-2 ring-[#D97706]/20' : 'border-gray-200'
                }`}
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-gray-100 pb-3">
                  <div className="flex items-center gap-2">
                    <div className="p-2 rounded-xl bg-[#7A2E1E] text-[#D97706]">
                      <Megaphone className="w-4 h-4 text-[#D97706]" />
                    </div>
                    <div>
                      <span className="font-bold text-sm text-[#3A1C14] block">{n.senderName}</span>
                ...
                <div className="space-y-2">
                  <h3 className="font-bold font-serif text-base text-[#3A1C14]">{n.title}</h3>
                ...
                <div className="pt-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  ...
                  {!n.isConfirmed && (
                    <button
                      ...
                      className="px-5 py-2.5 rounded-2xl bg-[#7A2E1E] hover:bg-[#3A1C14] text-white text-xs font-bold transition-all shadow-md flex items-center gap-2 cursor-pointer border border-[#D97706]/40"
                    >
                      <UserCheck className="w-4 h-4 text-[#D97706]" /> Xác Nhận Đã Đọc & Đã Hiểu
                    </button>
                  )}
                </div>
              </div>
