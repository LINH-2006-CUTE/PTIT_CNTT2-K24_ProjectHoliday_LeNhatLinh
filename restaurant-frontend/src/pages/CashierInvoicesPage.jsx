    ...
    try {
      ...
      if (res.data && res.data.success && res.data.data) {
        const list = res.data.data.map((o) => ({
          id: o.id,
          invoiceNumber: `INV-${String(o.id).padStart(6, '0')}`,
          customerName: o.customerName || 'Khách vãng lai',
          customerPhone: o.customerPhone || '—',
          tableName: o.tableName || 'Mang về',
          subtotal: o.totalAmount || (o.items || []).reduce((sum, item) => sum + (item.price || 0) * (item.quantity || 0), 0),
          discountAmount: 0,
          vatAmount: 0,
          totalAmount: o.totalAmount || 0,
          paymentMethod: o.paymentMethod || 'Chuyển Khoản QR / POS',
          issuedAt: o.orderDate ? new Date(o.orderDate).toLocaleString('vi-VN') : new Date().toLocaleString('vi-VN')
        }));
        setInvoices(list);
      } else {
        setInvoices([]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {loading ? (
          <div className="py-24 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">
            Đang tải danh sách hóa đơn VAT...
          </div>
