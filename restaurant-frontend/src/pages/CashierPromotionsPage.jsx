  ...
  const fetchPromotions = async () => {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        ...
        setPromotions(content.map(p => ({
          code: p.code || p.promotionCode || '',
          name: p.name || p.title || 'Mã ưu đãi',
          discount: p.discountPercent ? `${p.discountPercent}%` : p.discountAmount ? `${p.discountAmount} ₫` : 'Ưu đãi',
          status: p.status || (p.active ? 'ACTIVE' : 'EXPIRED'),
          validUntil: p.endDate || p.validUntil || '2026-12-31'
        })));
      }
    } catch (err) {
      console.error(err);
