...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // Ensure Chef User is ALWAYS created if missing
        if (userRepository.findByEmail("chef@restaurant.com").isEmpty()) {
            User chefUser = User.builder()
                    .email("chef@restaurant.com")
                    .password(passwordEncoder.encode("chefpassword"))
                    .fullName("Bếp Trưởng (Chef)")
                    .phone("+84 909090909")
                    .roles(Set.of(chefRole, staffRole))
                    .enabled(true)
                    .build();
            userRepository.save(chefUser);
            log.info("Chef user (chef@restaurant.com) seeded successfully.");
        }
        ...
        // 2.8 Clean up legacy short-named dishes with old low prices (e.g. 45, 65, 48)
        List<Dish> legacyDishes = dishRepository.findAll();
        for (Dish d : legacyDishes) {
            if ("Filet Mignon".equalsIgnoreCase(d.getName())) {
                d.setPrice(new BigDecimal("580000"));
                d.setImage("https://images.unsplash.com/photo-1558030006-450675393462?auto=format&fit=crop&w=600&q=80");
                dishRepository.save(d);
            } else if ("Lobster Thermidor".equalsIgnoreCase(d.getName())) {
                d.setPrice(new BigDecimal("750000"));
                d.setImage("https://images.unsplash.com/photo-1553240799-36bbf332a5c3?auto=format&fit=crop&w=600&q=80");
                dishRepository.save(d);
            } else if ("Ribeye Steak".equalsIgnoreCase(d.getName())) {
                d.setPrice(new BigDecimal("620000"));
                d.setImage("https://images.unsplash.com/photo-1600891964092-4316c288032e?auto=format&fit=crop&w=600&q=80");
                dishRepository.save(d);
            } else if (d.getPrice() != null && d.getPrice().compareTo(new BigDecimal("1000")) < 0) {
                // If any dish price is less than 1,000 VND (e.g. 45 or 65), multiply by 10,000 to get proper VND price
                d.setPrice(d.getPrice().multiply(new BigDecimal("10000")));
                dishRepository.save(d);
            }
        }
        
        // --- MÓN CHÍNH (MAIN COURSE) ---
        dishes.add(saveDish("Filet Mignon Bò Wagyu", new BigDecimal("580000"), "Thăn nội bò Wagyu sốt vang đỏ Bordeaux và nấm Truffle đen.", "https://images.unsplash.com/photo-1558030006-450675393462?auto=format&fit=crop&w=600&q=80", new BigDecimal("10"), mainCat));
        ...
        // 3.9 Comprehensive Dish Price & Image Sanitizer for Fine Dining Consistency
        List<Dish> allSeededDishes = dishRepository.findAll();
        for (Dish d : allSeededDishes) {
            String nameLower = d.getName() != null ? d.getName().toLowerCase() : "";
            
            // Fix legacy prices < 1000 VND
            if (d.getPrice() != null && d.getPrice().compareTo(new BigDecimal("1000")) < 0) {
                d.setPrice(d.getPrice().multiply(new BigDecimal("10000")));
            }

            // Fix missing or duplicate fallback images with realistic matching photos
            if (d.getImage() == null || d.getImage().trim().isEmpty() || d.getImage().contains("photo-1544025162-d76694265947")) {
                if (nameLower.contains("wagyu") || nameLower.contains("filet mignon")) {
                    d.setImage("https://images.unsplash.com/photo-1558030006-450675393462?auto=format&fit=crop&w=600&q=80");
                } else if (nameLower.contains("lobster") || nameLower.contains("tôm hùm")) {
                    d.setImage("https://images.unsplash.com/photo-1553240799-36bbf332a5c3?auto=format&fit=crop&w=600&q=80");
                } else if (nameLower.contains("ribeye") || nameLower.contains("steak") || nameLower.contains("bít tết")) {
                    d.setImage("https://images.unsplash.com/photo-1600891964092-4316c288032e?auto=format&fit=crop&w=600&q=80");
                } else if (nameLower.contains("salmon") || nameLower.contains("cá hồi")) {
                    d.setImage("https://images.unsplash.com/photo-1467003909585-2f8a72700288?auto=format&fit=crop&w=600&q=80");
                } else if (nameLower.contains("caesar") || nameLower.contains("salad")) {
                    d.setImage("https://images.unsplash.com/photo-1550304943-4f24f54ddde9?auto=format&fit=crop&w=600&q=80");
                } else if (nameLower.contains("fries") || nameLower.contains("khoai tây")) {
                    d.setImage("https://images.unsplash.com/photo-1573080496219-bb080dd4f877?auto=format&fit=crop&w=600&q=80");
                } else if (nameLower.contains("pasta") || nameLower.contains("spaghetti") || nameLower.contains("mì ý")) {
                    d.setImage("https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?auto=format&fit=crop&w=600&q=80");
                }
            }
            dishRepository.save(d);
        }
