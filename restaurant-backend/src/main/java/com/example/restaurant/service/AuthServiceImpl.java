...
public class AuthServiceImpl implements AuthService {
    ...
    public void register(RegisterRequest request) {
        ...
        if (userRepository.existsByEmail(cleanEmail)) {
            throw new ApiException("Email này đã được đăng ký!", HttpStatus.BAD_REQUEST);
        }

        String cleanPhone = (request.getPhone() != null && !request.getPhone().trim().isEmpty())
                ? request.getPhone().trim() : null;

        if (cleanPhone != null && !cleanPhone.isEmpty()) {
            if (userRepository.existsByPhone(cleanPhone) || customerRepository.findByPhone(cleanPhone).isPresent()) {
                throw new ApiException("Số điện thoại '" + cleanPhone + "' đã được sử dụng bởi một tài khoản khác!", HttpStatus.BAD_REQUEST);
            }
        }
        ...
        // Synchronize Customer card immediately upon registration
        Customer customer = Customer.builder()
                .fullName(user.getFullName())
                .email(user.getEmail())
                .phone(cleanPhone != null ? cleanPhone : "09" + String.format("%08d", System.currentTimeMillis() % 100000000L))
                .membership(true)
                .points(0)
                .rank("MEMBER")
                .build();
    ...
    public AuthResponse login(LoginRequest request) {
        ...
        return AuthResponse.builder()
                ...
                .phone(user.getPhone() != null ? user.getPhone() : userDetails.getPhone())
