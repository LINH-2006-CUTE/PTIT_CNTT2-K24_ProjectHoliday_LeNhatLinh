...
public class CashierServiceImpl implements CashierService {
    ...
    public List<OrderHistoryDTO> getPendingOrders() {
        ...
        return orders.stream()
                .filter(o -> {
                    ...
                    return st.equals("KITCHEN_CONFIRMED") || st.equals("COOKING") || st.equals("PREPARING")
                        || st.equals("READY") || st.equals("SERVED") || st.equals("IN_SERVICE")
                        || st.equals("CONFIRMED") || st.equals("PENDING"); // Phương án B: Thu ngân thấy đơn đang chế biến để chuẩn bị hóa đơn
                })
    ...
    public InvoiceDTO processCheckout(CashierCheckoutRequest request) {
        ...
        // Update Order status to WAITING_CUSTOMER_RECEIPT_CONFIRM (Cashier issued Invoice, waiting for Customer to confirm receipt)
        order.setStatus("WAITING_CUSTOMER_RECEIPT_CONFIRM");
    ...
    public CashierShiftReportDTO getShiftReport(String cashierEmail) {
        List<Order> completedOrders = orderRepository.findAll().stream()
                .filter(o -> o.getStatus() != null &&
                        ("COMPLETED".equalsIgnoreCase(o.getStatus()) ||
                         ...
                         "SERVED".equalsIgnoreCase(o.getStatus()) ||
                         "READY".equalsIgnoreCase(o.getStatus())))
                .collect(Collectors.toList());

        List<Invoice> invoices = invoiceRepository.findAll();

        Map<Long, BigDecimal> orderRevenueMap = new LinkedHashMap<>();
        for (Order o : completedOrders) {
            orderRevenueMap.put(o.getId(), o.getTotalAmount() != null ? o.getTotalAmount() : BigDecimal.ZERO);
        }
        for (Invoice inv : invoices) {
            if (inv.getOrder() != null && inv.getOrder().getId() != null) {
                orderRevenueMap.put(inv.getOrder().getId(), inv.getGrandTotal() != null ? inv.getGrandTotal() : BigDecimal.ZERO);
            }
        }

        BigDecimal totalRevenue = orderRevenueMap.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
        int invoiceCount = (int) Math.max(invoices.size(), completedOrders.size());

        BigDecimal cash = invoices.stream()
                .filter(i -> i.getPaymentMethod() != null && (i.getPaymentMethod().equalsIgnoreCase("CASH") || i.getPaymentMethod().contains("MẶT")))
                .map(Invoice::getGrandTotal)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal qr = invoices.stream()
                .filter(i -> i.getPaymentMethod() != null && i.getPaymentMethod().equalsIgnoreCase("QR"))
                .map(Invoice::getGrandTotal)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal vnpay = invoices.stream()
                .filter(i -> i.getPaymentMethod() != null && i.getPaymentMethod().equalsIgnoreCase("VNPAY"))
                .map(Invoice::getGrandTotal)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal momo = invoices.stream()
                .filter(i -> i.getPaymentMethod() != null && i.getPaymentMethod().equalsIgnoreCase("MOMO"))
                .map(Invoice::getGrandTotal)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal card = invoices.stream()
                .filter(i -> i.getPaymentMethod() != null && i.getPaymentMethod().equalsIgnoreCase("CARD"))
                .map(Invoice::getGrandTotal)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        if (totalRevenue.compareTo(invoiceTotalSum) > 0) {
            BigDecimal unassigned = totalRevenue.subtract(invoiceTotalSum);
            cash = cash.add(unassigned);
        }
        ...
        return CashierShiftReportDTO.builder()
                ...
                .momoTotal(momo)
                .cardTotal(card)
                .shiftStartTime(LocalDateTime.now().minusHours(8))
                .build();
    }
